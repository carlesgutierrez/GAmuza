...
void GLEditor::DrawErrorLine(int _line)
{
	if(m_haveError){
		glColor4f(0.8,0.1,0.1,0.6);
		glBegin(GL_QUADS);
		glVertex2f(0,-(m_CharHeight*(_line-1)));
		glVertex2f(0,-(m_CharHeight*_line));
		glVertex2f(m_AutoFocusWidth,-(m_CharHeight*_line));
		glVertex2f(m_AutoFocusWidth,-(m_CharHeight*(_line-1)));
		glEnd();
	}
}
