...
void audioInputChannel::setupChannel(int _id,int _sampleRate,int _bufferSize,int _nChannels,int fftWindow){
	...
	kchVolume[0]			= new ofxCvKalman(0.0f);
	kchPitch[0]				= new ofxCvKalman(0.0f);
	...
	for(unsigned int i=0;i<BARK_SCALE_CRITICAL_BANDS;i++){
		kbarkBins[i]	= new ofxCvKalman(0.0f);
	}
...
//
//--------------------------------------------------------------
void audioInputChannel::kalmanFilterValues(){
	if(useKalmanFilter){
		// volume detection
		_kalman_chVolume = kchVolume[0]->correct(chVolume);
		// pitch detection
		_kalman_chPitch = kchPitch[0]->correct(chPitch);
		// bark scale bins
		for(unsigned int i=0;i<BARK_SCALE_CRITICAL_BANDS;i++){
			_kalman_barkBins[i] = kbarkBins[i]->correct(barkBins[i]);
		}
	}else{
		// volume detection
		_kalman_chVolume = chVolume;
		// pitch detection
		_kalman_chPitch = chPitch;
		// bark scale bins
		for(unsigned int i=0;i<BARK_SCALE_CRITICAL_BANDS;i++){
			_kalman_barkBins[i] = barkBins[i];
		}
	}
}
//
...
void audioInputChannel::smoothingValues(){
	// volume detection
	_s_chVolume = _s_chVolume*_smoothingFactor + (1.0-_smoothingFactor)*_kalman_chVolume;
	// pitch detection
	_s_chPitch = _s_chPitch*_smoothingFactor + (1.0-_smoothingFactor)*_kalman_chPitch;
	// bark scale bins
	for(unsigned int i=0;i<BARK_SCALE_CRITICAL_BANDS;i++){
		_s_barkBins[i] = _s_barkBins[i]*_smoothingFactor + (1.0-_smoothingFactor)*_kalman_barkBins[i];
	}
}
//
